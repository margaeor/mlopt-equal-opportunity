...
if __name__ == '__main__':
    ...
    for i in range(1,5):
        ...
        if i < 3:
            ...
            final_mapper.update({intervention_cols[i]:mapper[val] for i, val in enumerate(intervention_vals)})
            print(final_mapper)
        else:
            df_tmp = pd.read_csv(os.path.join('data','weights','hol_map.csv'))
            final_mapper.update({a:b for _,(a, b) in df_tmp.loc[:, ['key', 'value']].drop_duplicates().iterrows() if not pd.isna(b) and 'nan' not in b})
    ...
    fig.write_image('exports/sparse2.svg',scale=2, width=1000, height=600)
    pyo.plot(fig)
